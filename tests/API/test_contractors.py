...
class TestContractorsApi:
    ...
    @allure.title("Архивация контрагента,тест специально будет падать с 200 ошибкой, хотя ожидаетсся 400")
    def test_archive_contractor(self):
        contractor2 = ContractorData(users=[{"id": 12331}])
        ...
        assert result.status_code == 400, f"Ошибка , код ответа {result.status_code,result.json()}"
